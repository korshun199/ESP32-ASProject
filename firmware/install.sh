...
cd /home/work/ESP32-ASProject || exit 1

echo "========== ВЕТКА =========="
git branch --show-current

cat > firmware/radar/radar.ino <<'EOF'
/*
...
echo
echo "========== ПРОВЕРКА НОВОГО ПРОТОКОЛА =========="
grep -nE 'FRAME_TARGET|CORR_MASK|EFFECTIVE_K|BACKGROUND_SCORE|TONE400_AMP|SEND_T_TO_START' firmware/radar/radar.ino

echo
echo "========== РАЗМЕР И ХЭШ =========="
wc -l -c firmware/radar/radar.ino
sha256sum firmware/radar/radar.ino

echo
echo "========== СБОРКА И ПРОШИВКА =========="
./scripts/run.sh
